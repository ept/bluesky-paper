...
\begin{abstract}
    ...
    It was launched in private beta in February 2023, and has grown to over 6 million registered users by July 2024.
...
Bluesky launched an invite-only beta in February 2023, and has grown to over 6~million registered users in the following 18 months. % as shown in Figure~\ref{fig:user-growth}.
...
% \begin{itemize}
    ...
    % \item
        For organizations and people that already have a well-known domain name, using that name makes it easy for users to check that their Bluesky account is genuine. For example, the New York Times' handle is \texttt{@nytimes.com}.
    % \item
...
Mastodon advertises itself as having ``no algorithms or ads to waste your time''~\cite{Mastodon}.
...
The AT Protocol does not make any assumptions about how PDSes authenticate their users; other PDS operators are free to use different methods.
