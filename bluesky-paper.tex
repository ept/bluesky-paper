...
\keywords{social media, decentralization, federation, web architecture, community moderation}
\maketitle
...
Our hope is that this architectural openness enables communities to develop their own approaches to managing problematic content, independently of what any particular service operator implements.
...
\begin{description}
    ...
    \item[Interaction gating:] A user who makes a post can restrict who is allowed to reply to it (anyone, anyone they follow, anyone mentioned in the post, and/or anyone on a particular list of accounts).
...
This allows different sets of people to offer server hosting and moderation services, respectively; we believe this separation is valuable since operating a server and moderating a community require largely disjoint sets of skills.
...
BGS and App View aim to provide a mostly ``unopinionated'' service: they compute indexes over repositories in a neutral way, without attempting to rank or classify content.
...
In 2021 some of our team published a decentralized social ecosystem review~\cite{EcosystemReview} in preparation for designing Bluesky/atproto.
In this section we summarize some of the recent developments in other decentralized social networks that have happened since, and we refer to the review for a more comprehensive comparison of protocols and systems.
...
An atproto PDS has similarities to Git repository hosting (e.g.\ GitHub/Gitlab), Solid pods~\cite{Solid}, or Mastodon servers.
...
Communication (e.g.\ reply threads) is only possible between users who are connected to the same relay.
Although some services index the whole Nostr network, these indexes are not used to facilitate user-to-user interaction.
...
It has storage servers called \emph{hubs}, which store the state of the entire network similarly to an atproto BGS, and it has a concept of \emph{hosted app servers} that are similar to our App View~\cite{FarcasterOverview}.
...
Overall, Mastodon gives a lot of power to server administrators: for example, a server admin can choose to block another server, preventing all communication between users on those servers, even those who are not part of the disagreement that led to the blocking.
There is a degree of lock-in to a server because moving to another server is an intrusive operation: the username changes, moving posts to the new server currently requires an experimental command-line tool~\cite{MastodonPostMigration,MastodonContentMover}, and other users' replies to those post are lost.
These risks can be avoided by self-hosting; managed providers exist~\cite{MastodonHosting}, but they still require some expertise and cost money.
...
Ensuring consistent reply threads would require each server to have a copy of the whole network, which would make it expensive to run a server.
Viral posts can generate a lot of inbound requests to a server from people liking, replying, and boosting (reposting).
The Bluesky indexing infrastructure is also fairly expensive, but since users can choose their moderation preferences independently from the indexing provider, we believe that the system can be healthy with a small number of indexing providers.

Security in ActivityPub is based on servers' TLS certificates; there is no per-user cryptographic authentication of content.
