...
We believe that there is no single optimal design: it is important to have several systems that make different trade-offs, and are therefore suitable for different purposes.
Bluesky and the AT Protocol's choice of trade-offs aims to have lightweight clients while providing a global view over the whole network; for example, conversation threads include all replies (unless removed by moderation), regardless of the server on which they were posted.
To achieve this goal we rely on an indexing infrastructure that is more centralized than some other designs (although we emphasize that there can be multiple competing indexers).
Other systems make different trade-off choices, which are equally valid.
...
An atproto PDS has similarities to Git repository hosting (e.g.\ GitHub/Gitlab), Solid pods, or Mastodon servers.
Public key authentication of user actions is widely used.
...
The protocol is deliberately simple, and it prioritizes censorship resistance over moderation: relays can block users, but users can always move to a new relay.
...
We also prefer to avoid a dependency on a blockchain or cryptocurrency.
